...
export function ProductCard({ product }: { product: Product }) {
  const currencyFormatter = new Intl.NumberFormat('es', { currency: 'USD', style: "currency" })
  ...
  return (
    <div className="relative overflow-hidden rounded-xl shadow-sm font-sans bg-white">
      ...
      <div className="relative w-full h-64">
        <img className="object-cover w-full h-full" src={`https://tiendasdaka.com/img/producto/${product.sap}.webp`} alt={product.descripcion} />
      ...
      <div className="p-4">
        <p className="text-stone-500 line-clamp-2 text-sm mt-2">{product.marca}</p>
        ...
        <div className="flex items-center justify-between mt-4">
          {product.promo ? (
            ...
            <p className="text-lg text-neutral-600">{price}</p>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <section className="py-12 sm:py-14 lg:py-16">
      <div className="mx-auto max-w-7xl px-4">
        <p className="text-2xl text-blue-800/90 font-bold text-center md:text-3xl">Productos pensados para ti</p>
